...
def load_csv_ccdf(csv_file):
    ...
    df = df.dropna(subset=["hmm_max_cfr"])
    df["hmm_max_cfr"] = df["hmm_max_cfr"].astype(float)
    ...
    print(intra_as)
    intra_as_data, intra_as_cdf = compute_cdf(intra_as["hmm_max_cfr"])
    inter_as_data, inter_as_cdf = compute_cdf(inter_as["hmm_max_cfr"])
...
if __name__ == "__main__":
    input_csv = (
        "../../data/csv/hmm-as-max-cfr-statistics.csv"  # Change to actual CSV file path
    )
    output_pdf = "../../data/plots/hmm-as-max-cfr-ipinfo-cdf.pdf"
