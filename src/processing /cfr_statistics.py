...
all CFR related plots. Replace hmm_hdp with ranks or bootstrap
...
def compute_statistics(timeline_data, cp_func):
    ...
    cps.sort()
    cps_diff = [
        AVG_SPREAD * cps[i] - AVG_SPREAD * cps[i - 1] for i in range(1, len(cps))
    ]
...
if __name__ == "__main__":
    ...
    csv_filename = "../../data/csv/hmm-as-cfr-ipinfo-statistics.csv"
    ...
    with open(csv_filename, mode="w", newline="") as file:
        ...
        for (anchor_id, dst_name), value in timeline_dict.items():
            hmm_statistics_info = compute_statistics(
                timeline_dict[(anchor_id, dst_name)], cp_hmm_hdp
            )
            count += 1
            print(count)

            for idx, point in enumerate(hmm_statistics_info):
                writer.writerow(
                    [
                        anchor_id,
                        dst_name,
                        asn_continent_dict[(anchor_id, dst_name)][0],
                        asn_continent_dict[(anchor_id, dst_name)][1],
                        idx,
                        float(point),
                    ]
                )
